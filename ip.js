...
// 预设 Cloudflare 应急保活 IP 列表（绝不触发 500 崩溃）
const EMERGENCY_IPS = [
    '1.1.1.1',
    '1.0.0.1',
    '104.16.132.229',
...
// 核心入口：同时支持 GET (Next.js App Router) 与 default (Serverless API)
export async function GET(req, res) { return handleRequest(req, res); }
export default async function handler(req, res) { return handleRequest(req, res); }

async function handleRequest(req, res) {
    try {
        const { body, status, cacheHeader } = await processCoreLogic();
        return sendResponse(res, body, status, cacheHeader);
    } catch (fatalError) {
        // 兜底保活：即使发生致命未知异常，也绝对不报错
        return sendResponse(res, cache.data || EMERGENCY_IPS.join('\n'), 200, 'FATAL-RECOVERY');
    }
}

// 统一安全响应分配器（兼顾 Node req/res 与 Web Standard Response）
function sendResponse(res, body, status = 200, cacheHeader = 'MISS') {
    const headers = {
    ...
    // 2. Next.js App Router / Edge / Web Standard 格式
    return new Response(body, { status, headers });
}

// 核心业务逻辑
async function processCoreLogic() {
    const now = Date.now();

    // 命中缓存
    if (cache.data && (now - cache.timestamp) < cache.ttl) {
        return { body: cache.data, status: 200, cacheHeader: 'HIT' };
    }
    ...
    if (uniqueIPs.length === 0) {
        if (cache.data) {
            return { body: cache.data, status: 200, cacheHeader: 'HIT-FALLBACK' };
        } else {
    ...
    return { body: resultText, status: 200, cacheHeader: cacheStatus };
}
